...
def run(
    ...
    s = ('%20s' + '%11s' * 5) % ('Class', 'Images', 'Labels', 'P', 'R', 'mAP@.5')
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # out 输出和 c++ 中nms的形式一致，六维向量，【xy, xy, conf, cls】

        # Metrics
        # si 是一个batch的图片索引
        for si, pred in enumerate(out): # 遍历每一个batch
            # 得到该图片的所有，label 信息
    ...
    print(pf % ('all', seen, nt.sum(), mp, mr, map50))

    # Print results per class
    for i, c in enumerate(ap_class):
        LOGGER.info(pf % ('car', seen, nt[c], p[i], r[i], ap50[i]))
    # Print speeds
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / '/mnt21t/home/wyh/wyh_project/bdd100k-yolov5/BDD100/yolo_dataset/bdd_yolo.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weights/ghost_relu3.pt', help='model.pt path(s)')
