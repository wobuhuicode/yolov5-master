...
def run():
    ...
    for i in range(len(model_names)):
        ...
        if os.path.exists(f'ccpd_val_out/{model_names[i]}_ref') is False:
            ...
            os.mkdir(f'ccpd_val_out/{model_names[i]}_ref/preds')
        ...
        out_dir = f'ccpd_val_out/{model_names[i]}_ref'
        ...
        all_inference_result = np.loadtxt(f"ccpd_valid_output/{model_names[i]}_out.txt", dtype=np.float64, delimiter=',')
