...
def run():
    ...
    all_inference_result = np.loadtxt("/mnt/d/Littro_3519A/ubuntu/bdd-expr-on-board/release/valid_out/ghost_relu2_out.txt", dtype=np.float64, delimiter=',')
    ...
    for si in tqdm(range(1, 10001, 1)): # 遍历
        ...
        # 图片的 路径，图片的形状
        path = f"/mnt/d/Littro_3519A/ubuntu/bdd-expr-on-board/bdd_val/images/{si}.jpg"
        ...
        # img = cv2.imread(f"/mnt/d/Littro_3519A/ubuntu/bdd-expr-on-board/bdd_val/images/{si}.jpg")

        # for i in range(len(predn)):
        #     cv2.rectangle(img, (int(predn[i, 0]), int(predn[i, 1])), (int(predn[i, 2]), int(predn[i, 3])), (0, 0, 255), 2)

        # cv2.imwrite(f"bdd_val_out/out{si}.jpg", img)
        ...
        if nl:
            ...
            #     cv2.rectangle(img, (int(tbox[i, 0]), int(tbox[i, 1])), (int(tbox[i, 2]), int(tbox[i, 3])), (0, 0, 255), 2)

            # cv2.imwrite("bdd_val_out/label.jpg", img)
    ...
    if len(stats) and stats[0].any():
        ...
        tp, fp, p, r, f1, ap, ap_class = ap_per_class(*stats, plot=plots, save_dir="bdd_val_out/", names=names)
    ...
    # Plots
    if plots:
        confusion_matrix.plot(save_dir="bdd_val_out/", names=list(names.values()))
